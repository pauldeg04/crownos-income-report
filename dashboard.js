...
/* Any logged-in Therapist or Receptionist can receive notifications —
   ...
   schedule assignments identify them), Receptionists (and anyone else)
   by their login account, since that's the only identity they have. */
...
/* Populated live by CrownServerNotifications.listenForUser() — merged
   with CrownNotifications' local list wherever notifications are read
   below (see getMergedNotifications). Not persisted anywhere else; a
   page reload just re-subscribes. */
...
function initializeNotificationBell(){
    ...
    const wrap =
        document.getElementById("notificationBellWrap");
    ...
    document
        .getElementById("notificationMarkAllBtn")
    ...
    window.addEventListener("crownCloudUpdate", function(event){
        if(
            event.detail?.keys?.includes(CROWN_NOTIFICATIONS_KEY)
        ){
            updateNotificationBadge(recipient);
...
/* Combines the client-owned local list (CrownNotifications, mirrored from
   localStorage) with the live server-created list (staffNotifications,
   e.g. new booking requests for a Receptionist — see notifications.js /
   notifyReceptionistsOnNewBookingRequest), newest first. Server entries
   are tagged source:"server" so mark-read routes to the right backend. */
